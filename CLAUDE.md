...
| Subscriptions | `com.awdia.podradar.pro.weekly` — **launch at 2,99 €/wk** (matches PodSpot's live paywall price, the proven revenue benchmark — see app-marketing-context.md), 3-day free trial; `com.awdia.podradar.pro.weekly.full` — same price/period, NO trial (Paywall A); `com.awdia.podradar.pro.yearly` — 29,99 €/yr anchor. Store layer is **RevenueCat** (2026-07-20, same pattern as RepLock — see its CLAUDE.md 4E) — see "RevenueCat" row below. |
| RevenueCat | Same account as RepLock/Loopa, **separate project** "PodRadar" (id `projc6c67ac3`), scripted entirely via the Management API (secret key never committed — used once, locally, to script the setup). Entitlement `pro` attached to all 3 ASC products above. Offering `default` (`is_current`) has 2 packages: `weekly_trial` → `com.awdia.podradar.pro.weekly`, `weekly_full_price` → `com.awdia.podradar.pro.weekly.full`. App `PodRadar iOS` (id `app794a77da17`), bundle `com.awdia.podradar` — **App Store Connect API key NOT yet linked** (`app_store_connect_api_key_configured: false`), needed before RevenueCat can validate real receipts/webhooks; sandbox purchases may still work via device receipt but link it before relying on server-side entitlement checks. Public SDK key is in `SubscriptionManager.revenueCatAPIKey` (safe to ship, unlike the secret key). `SubscriptionOffer` is the view-shaped type paywalls read — they never import RevenueCat directly. |
